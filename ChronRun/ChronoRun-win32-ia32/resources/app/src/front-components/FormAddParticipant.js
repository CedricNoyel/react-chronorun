...
class FormAddParticipant extends Component {
    ...
    onFormSubmitted() {
        if (this.state.inputParticipant.length !== 0) {
            if (this.props.listeParticipants.find( elem => elem.dossard === this.state.inputParticipant) === undefined) {
                ...
                openSnackbar({message: "Erreur, le dossard est déjà assigné !"}, {type: "error"})
            }
        } else {
            openSnackbar({message: "Erreur, le dossard doit être renseigné !"}, {type: "error"})
        }
    }
